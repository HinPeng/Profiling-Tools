...
def benchmark_one_step(sess, fetches, step, batch_size,
  ...
  if step >= 0 and (step == 0 or (step + 1) % FLAGS.display_every == 0):
    log_fn('%i\t%s\t%.3f' % (
        step + 1, get_perf_timing_str(step, batch_size, step_train_times),
        lossval))
  if trace_filename is not None and step == -1:
...
def get_perf_timing_str(step, batch_size, step_train_times, scale=1):
  times = np.array(step_train_times)
  speeds = batch_size / times
  speed_mean = scale * batch_size / times[step]
  if scale == 1:
...
class BenchmarkCNN(object):
  ...
  def _benchmark_cnn(self):
    ...
    with sv.managed_session(
      ...
      total_times=np.array(step_train_times)
      avg_speed=len(self.worker_hosts) * self.batch_size / np.mean(total_times[1:100])
      log_fn('Avg images/sec: %.2f' % avg_speed)
